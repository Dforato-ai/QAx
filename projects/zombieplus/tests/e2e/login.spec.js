...
test('não deve logar com senha incorreta', async ({ page }) => {
    ...
    await loginPage.visit()
    await loginPage.submit('admin@zombieplus.com', 'xxx123')
...
test('não deve logar quando o email é incorreto', async ({ page }) => {
    ...
    await loginPage.visit()
    await loginPage.submit('www.ad', 'abc123')
...
test('não deve logar quando a password não é preenchido', async ({ page }) => {

    await loginPage.visit()
    await loginPage.submit('papito@hotmail.com', '')
    await loginPage.alertHaveText('Campo obrigatório')
})
